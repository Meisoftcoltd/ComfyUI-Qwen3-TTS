...
def fix_wsl_path(path):
    ...
    if platform.system() == "Linux":
        ...
        if len(path) >= 2 and path[1] == ":" and path[0].isalpha():
            ...
            print(
                f"[Qwen3-TTS DEBUG] Detected Windows path in WSL: '{path}' -> converting to '{new_path}'"
            )
            return new_path
    return path
...
def get_local_model_path(repo_id: str) -> str:
    ...
    folder_name = (
        QWEN3_TTS_MODELS.get(repo_id)
        or QWEN3_TTS_TOKENIZERS.get(repo_id)
    ...
    if os.path.exists(hf_model_dir):
        ...
        if os.path.exists(snapshots_dir):
            ...
            if snapshots:
                ...
                print(
                    f"Migrating model from HuggingFace cache: {source} -> {target_path}"
                )
...
def load_audio_input(audio_input):
    ...
    print(
        f"load_audio_input received: waveform.shape={waveform.shape}, waveform.dim={waveform.dim()}"
    )
    ...
    if waveform.dim() == 0:
        ...
        raise ValueError(
            f"Waveform is 0-dimensional (scalar tensor). This causes 'Len() of unsized object' error."
        )
        ...
        raise ValueError(
            f"Unexpected waveform dimension: {waveform.dim()} (shape: {waveform.shape})"
        )
            ...
            "optional": {
                "local_model_path": (
                    "STRING",
                    {
                        "default": "",
                        "multiline": False,
                        "tooltip": "Path to local model or checkpoint. If checkpoint (no speech_tokenizer/), base model loads from repo_id first.",
                    },
                ),
            },
        }
    ...
    def load_model(self, repo_id, source, precision, attention, local_model_path=""):
        ...
        if local_path_stripped:
            ...
            if os.path.exists(p1):
                ...
                target_path = p1  # Default fallback
        else:
            # Repo ID logic
        ...
        if os.path.exists(speech_tokenizer_path) and os.path.exists(
            os.path.join(speech_tokenizer_path, "config.json")
        ):
            # It's a full model
            ...
            print(
                f"Target path '{target_path}' appears to be a checkpoint (missing speech_tokenizer)."
            )
            ...
            if os.path.exists(local_base) and os.path.exists(
                os.path.join(local_base, "speech_tokenizer")
            ):
                model_path = local_base
            else:
                print(
                    f"Downloading default base model {default_base} for checkpoint overlay..."
                )
        ...
        try:
            cfg_file = (
                ...
                if checkpoint_path
                else os.path.join(model_path, "config.json")
            )
    ...
    def load_model(self, base_model, fine_tuned_model, precision, attention):
        ...
        if clean_base in QWEN3_TTS_MODELS or clean_base in QWEN3_TTS_MODELS.values():
            ...
            for p in potential_paths:
                if os.path.exists(p) and os.path.exists(
                    os.path.join(p, "speech_tokenizer")
                ):
                    base_model_path = p
                    break
            ...
            if not base_model_path:
                ...
                print(
                    f"[Qwen3-TTS] Warning: '{clean_base}' does not appear to be a valid base model (missing speech_tokenizer). Falling back to default Base."
                )
                ...
                if os.path.exists(local_default):
                    ...
                    base_model_path = download_model_to_comfyui(
                        default_repo, "HuggingFace"
                    )
        ...
        if os.path.exists(ckpt_weights):
            ...
            if keys.missing_keys:
                print(
                    f"DEBUG: Missing keys (expected for PEFT or partial loads, check if critical): {keys.missing_keys[:5]}..."
                )
            if keys.unexpected_keys:
                print(f"DEBUG: Unexpected keys: {keys.unexpected_keys[:5]}...")
        else:
            raise ValueError(
                f"pytorch_model.bin not found in checkpoint: {checkpoint_path}"
            )
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "temperature": (
                    "FLOAT",
                    {"default": 0.7, "min": 0.1, "max": 2.0, "step": 0.01},
                ),
                "repetition_penalty": (
                    "FLOAT",
                    {
    ...
    def generate(
        ...
        if use_masquerade:
            ...
            if cfg_ref and hasattr(cfg_ref, "spk_id"):
                ...
                # Find ID of our custom speaker (e.g., 3000)
                custom_id = original_spk_id_map.get(target_speaker)

                # Try case-insensitive lookup if exact match fails
                if custom_id is None:
                     for k, v in original_spk_id_map.items():
                         if k.lower() == target_speaker.lower():
                             custom_id = v
                             break

                if custom_id is not None:
                    # THE SWAP: Point 'Vivian' to ID 3000
                    cfg_ref.spk_id[masquerade_target] = custom_id
                    print(f"[Qwen3-TTS] 🔧 Masquerade active: '{masquerade_target}' -> ID {custom_id}")
                    # trick the library into accepting the call
        ...
        try:
            # Call generation (asking for 'Vivian' if masquerading, but getting Custom Voice)
            try:
                ...
                # Fallback for older library versions without kwargs
                print(
            ...
            # CRITICAL: Restore the config to avoid polluting the model state
            if use_masquerade and original_spk_id_map and cfg_ref:
                cfg_ref.spk_id = original_spk_id_map
                print("[Qwen3-TTS] 🛡️ Config restored.")
        ...
        return {
            ...
            "optional": {
                ...
                "pitch": (
                    "STRING",
                    {
                        "default": "Deep and resonant with subtle downward inflections suggesting gravity",
                        "multiline": True,
                    },
                ),
                "speed": (
                    "STRING",
                    {
                        "default": "Deliberately slow with extended pauses between sentences",
                        "multiline": True,
                    },
                ),
                "volume": (
                    "STRING",
                    {
                        "default": "Moderate to soft, creating an intimate atmosphere",
                        "multiline": True,
                    },
                ),
                "age": (
                    "STRING",
                    {"default": "Middle-aged to older adult", "multiline": False},
                ),
                "clarity": (
                    "STRING",
                    {
                        "default": "Crystal clear enunciation with careful articulation",
                        "multiline": True,
                    },
                ),
                "fluency": (
                    "STRING",
                    {
                        "default": "Smooth and controlled with intentional dramatic pauses",
                        "multiline": True,
                    },
                ),
                "accent": (
                    "STRING",
                    {"default": "Standard American English", "multiline": False},
                ),
                "texture": (
                    "STRING",
                    {
                        "default": "Rich and velvety with a slightly smoky quality",
                        "multiline": True,
                    },
                ),
                "emotion": (
                    "STRING",
                    {"default": "Contemplative and intriguing", "multiline": True},
                ),
                "tone": (
                    "STRING",
                    {
                        "default": "Mysterious, philosophical, and atmospheric",
                        "multiline": True,
                    },
                ),
                "personality": (
                    "STRING",
                    {
                        "default": "Introspective, wise, and captivating",
                        "multiline": True,
                    },
                ),
                "custom_instruction": (
                    "STRING",
                    {
                        "default": "",
                        "multiline": True,
                        "tooltip": "Any additional custom instructions to append.",
                    },
                ),
                "top_p": (
                    "FLOAT",
        ...
        try:
            try:
                wavs, sr = model.generate_voice_design(
                    text=text, language=lang, instruct=instruct, **gen_kwargs
                )
            except TypeError:
                print(
                    "Warning: Model generation function does not support extra parameters (top_p, temperature, repetition_penalty). Ignoring them."
            ...
            if "does not support generate_voice_design" in msg:
                raise ValueError(
                    "Model Type Error: You are trying to use 'Voice Design' with an incompatible model. Please load a 'VoiceDesign' model (e.g. Qwen3-TTS-12Hz-1.7B-VoiceDesign)."
                ) from e
            raise e
    ...
    def create_prompt(
        ...
        if audio_tuple is not None and ref_audio_max_seconds > 0:
            ...
            if len(wav_data) > max_samples:
                print(
                    f"Trimming reference audio from {len(wav_data)/audio_sr:.1f}s to {ref_audio_max_seconds}s to prevent generation issues"
                )
        ...
        try:
            if prompt is not None:
                ...
                try:
                    ...
                    print(
                        "Warning: Model generation function does not support extra parameters. Ignoring them."
                    )
                    wavs, sr = model.generate_voice_clone(
                        text=text,
                        language=lang,
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "max_duration": (
                    "FLOAT",
                    {"default": 15.0, "min": 1.0, "max": 30.0, "step": 0.5},
                ),
                "silence_threshold": (
                    "FLOAT",
    ...
    def process(
        ...
        def trim_silence_wrapper(audio_segment, thresh, chunk=10):
            try:
                start_trim = silence.detect_leading_silence(
                    audio_segment, silence_threshold=thresh, chunk_size=chunk
                )
                end_trim = silence.detect_leading_silence(
                    audio_segment.reverse(), silence_threshold=thresh, chunk_size=chunk
                )
        ...
        for idx, filename in enumerate(files):
            ...
            for segment in result["segments"]:
                ...
                chunk_trimmed.set_frame_rate(22050).set_channels(1).export(
                    wav_path, format="wav"
                )
        ...
        print(
            f"[Qwen3-TTS] Process finished. Generated {total_clips} clips in {output_folder}"
        )
            ...
            for filename in os.listdir(output_folder):
                if filename.endswith(".wav"):
                    ...
                    if os.path.exists(txt_path):
                        ...
                        if text_content:
                            entry = {"audio": wav_path, "text": text_content}
...
class Qwen3TranscribeWhisper:
    ...
    def process(
        ...
        processed_wavs = [
            f for f in os.listdir(processed_dir) if f.lower().endswith(".wav")
        ]

        if not processed_wavs:
            print(
                f"[Qwen3-TTS] No processed files found in {processed_dir}. Starting Whisper processing..."
            )
            ...
            print(
                f"[Qwen3-TTS] Found {len(processed_wavs)} existing processed files. Skipping Whisper."
            )
...
class Qwen3AutoLabelEmotions:
        ...
        return {
            "required": {
                ...
                "gender_override": (
                    ...
                    {
                        "default": "Auto",
                        "tooltip": "Force a specific gender tag. Changes 'Male voice' to 'Female voice' (or vice versa) in the description.",
                    },
                ),
                "quantization": (
                    ["4bit", "8bit", "none"],
                    {
                        "default": "4bit",
                        "tooltip": "Load model in 4-bit/8-bit to save VRAM. 4bit is highly recommended for speed.",
                    },
                ),
                "batch_size": (
                    "INT",
                    {
                        ...
                        "min": 1,
                        "max": 64,
                        "tooltip": "Number of audio files to process in parallel. Higher = faster but uses more VRAM.",
                    },
                ),
            },
            "hidden": {
    ...
    def label_emotions(self, dataset_items, model, gender_override, quantization, batch_size, unique_id=None):
        import re
        import torch
        ...
        print(
            f"[Qwen3-TTS] Auto-Labeling Emotions ({len(dataset_items)} items). Mode: {gender_override}, Quant: {quantization}, Batch: {batch_size}"
        )

        # Keyword mapping
        keyword_map = {
            "angry": "very angry, shouting, aggressive tone, high pitch.",
            "enfadado": "very angry, shouting, aggressive tone, high pitch.",
            "sad": "very sad, crying, low pitch, melancholic tone.",
            "triste": "very sad, crying, low pitch, melancholic tone.",
            "happy": "happy, laughing, bright tone, excited.",
            "feliz": "happy, laughing, bright tone, excited.",
            "whisper": "whispering, quiet, soft tone.",
            "susurro": "whispering, quiet, soft tone.",
            "scared": "scared, trembling, high pitch, fearful.",
            "miedo": "scared, trembling, high pitch, fearful.",
            "surprised": "surprised, shocked, high pitch.",
            "sorpresa": "surprised, shocked, high pitch.",
        }
        ...
        for idx, item in enumerate(dataset_items):
            ...
            if not instruction:
                ...
                for kw, kw_instruction in keyword_map.items():
                    if kw in path_lower:
                        # Apply gender immediately to keyword default
                        prefix = "Male voice, " if gender_override != "Female" else "Female voice, "
                        if gender_override == "Auto": prefix = "Male voice, " # Default fallback
                        
                        instruction = prefix + kw_instruction
        ...
        # --- PHASE 2: Inference (Batched & Quantized) ---
        if items_to_infer:
            ...
            try:
                ...
                model_obj = Qwen2AudioForConditionalGeneration.from_pretrained(model, **model_kwargs)
            ...
            with tqdm(total=total_infer, desc="Labeling (Batched)") as pbar:
                for i in range(0, total_infer, batch_size):
                    ...
                    for item in batch_items:
                        ...
                        try:
                            # Load audio
                            wav, sr = librosa.load(file_path, sr=processor.feature_extractor.sampling_rate)
                            audios.append(wav)
                            ...
                            # We keep it in batch_items but won't process it in model
                    ...
                    try:
                        ...
                        valid_idx = 0
                        for item in batch_items:
                            if "instruction" in item: continue # Skip the ones that failed loading
                            
                            raw_output = batch_outputs[valid_idx].strip()
                            final_instruction = apply_gender_fix(raw_output, gender_override)
                            
                            item["instruction"] = final_instruction
                            
                            # Cache result
                            try:
                                label_path = os.path.splitext(item["audio_path"])[0] + ".emotion.txt"
                                with open(label_path, "w", encoding="utf-8") as f:
...
class Qwen3DataPrep:
        ...
        return {
            "required": {
                ...
                "audio_tokenizer_repo": (
                    list(QWEN3_TTS_TOKENIZERS.keys()),
                ...
                "text_tokenizer_repo": (
                    "STRING",
                    {
                        ...
                        "tooltip": "Repo for the text tokenizer (e.g. Qwen2.5-7B-Instruct)",
                    },
                ),
                "source": (["HuggingFace", "ModelScope"], {"default": "HuggingFace"}),
                "batch_size": (
                    "INT",
                    {
                        ...
                        "min": 1,
                        "max": 32,
                        "tooltip": "Number of audio files to process at once. Lower values use less VRAM.",
                    },
                ),
            },
            "hidden": {
    ...
    def process(
        ...
        if os.path.exists(output_path):
            ...
            if metadata:
                if (
                    ...
                    and metadata.get("audio_tokenizer") == audio_tokenizer_repo
                    and metadata.get("output_line_count") == input_line_count
                ):
                    # Verify output file integrity
                    if count_jsonl_lines(output_path) == metadata.get(
                        "output_line_count"
                    ):
                        print(
                            f"[Qwen3DataPrep] Cache hit - using existing processed data"
                        )
        ...
        if os.path.exists(local_path) and os.listdir(local_path):
            ...
            print(
                f"Audio Tokenizer not found locally. Downloading {audio_tokenizer_repo}..."
            )
        ...
        if os.path.exists(text_tok_local) and os.listdir(text_tok_local):
            ...
            print(
                f"Text Tokenizer not found locally. Downloading {text_tokenizer_repo}..."
            )
            ...
            for batch_idx, batch in enumerate(batched_jsonl_reader(jsonl_path, batch_size)):
                ...
                try:
                    ...
                    if (
                        ...
                        or "allocation on device" in err_msg.lower()
                    ):
                        print(
                            f"[Qwen3DataPrep] ⚠️ OOM detected in batch {batch_idx+1}. Switching to sequential processing for this batch."
                        )
                        send_status(
                            f"⚠️ OOM in batch {batch_idx+1}. Retrying sequentially..."
                        )
                        ...
                        for b_item in batch:
                            try:
                                ...
                                enc_res_single = audio_tokenizer.encode(
                                    [b_item["audio"]]
                                )
                                ...
                                print(
                                    f"[Qwen3DataPrep] ❌ Failed to process file {b_item['audio']} even individually: {inner_e}"
                                )
...
class Qwen3FineTune:
        ...
        return {
            "required": {
                "train_jsonl": (
                    "STRING",
                    {
                        "default": "",
                        "multiline": False,
                        "tooltip": "Path to the preprocessed JSONL file containing training data with audio codes.",
                    },
                ),
                "init_model": (
                    all_models,
                ...
                "source": (
                    ...
                    {
                        "default": "HuggingFace",
                        "tooltip": "Download source if model is not found locally.",
                    },
                ),
                "output_dir": (
                    "STRING",
                    {
                        "default": "models/tts/finetuned_model",
                        "multiline": False,
                        "tooltip": "Directory to save checkpoints and final model.",
                    },
                ),
                "epochs": (
                    "INT",
            ...
            "optional": {
                ...
                "resume_training": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Continue training from the latest checkpoint in output_dir.",
                    },
                ),
                "log_every_steps": (
                    "INT",
                ...
                "mixed_precision": (
                    ...
                    {
                        "default": "bf16",
                        "tooltip": "bf16 recommended. Use fp32 only if GPU doesn't support bf16 (pre-Ampere).",
                    },
                ),
                ...
                "gradient_checkpointing": (
                    ...
                    {
                        "default": True,
                        "tooltip": "Trade compute for VRAM by recomputing activations. Saves ~30-40% VRAM.",
                    },
                ),
                "use_8bit_optimizer": (
                    "BOOLEAN",
                    {
                        "default": True,
                        "tooltip": "Use 8-bit AdamW optimizer. Saves ~50% optimizer VRAM. Requires bitsandbytes.",
                    },
                ),
                # Training Dynamics
                "weight_decay": (
                    "FLOAT",
                    {
                        ...
                        "max": 1.0,
                        "step": 0.001,
                        "tooltip": "L2 regularization strength to prevent overfitting.",
                    },
                ),
                "max_grad_norm": (
                    "FLOAT",
                ...
                "save_optimizer_state": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Save optimizer/scheduler state in checkpoints. Enables perfect resume but doubles checkpoint size.",
                    },
                ),
            },
            "hidden": {
    ...
    def train(
        ...
        print(
            f"[Qwen3-TTS DEBUG] Train called. JSONL: {train_jsonl}, Output: {output_dir}"
        )
        print(
            f"[Qwen3-TTS DEBUG] Config: Epochs={epochs}, Batch={batch_size}, LR={lr}, MixedPrecision={mixed_precision}"
        )
        ...
        if not os.path.isabs(output_dir):
            base_output_dir = os.path.abspath(
                os.path.join(folder_paths.base_path, output_dir)
            )
        else:
        ...
        if resume_training:
                ...
                if os.path.exists(direct_weights):
                    ...
                    if dir_name.startswith("ckpt_step_"):
                        try:
                            ...
                            pass
                    print(
                        f"Resume: output_dir is a checkpoint, loading from {resume_checkpoint_path}"
                    )
            ...
            if resume_checkpoint_path and resume_from_step == 0:
                training_config_path = os.path.join(
                    resume_checkpoint_path, "training_config.json"
                )
        ...
        with torch.inference_mode(mode=False):
            with torch.enable_grad():
                ...
                if (
                    ...
                    if device_cap[0] < 8:
                        print(
                            f"Warning: {gpu_name} (compute {device_cap[0]}.{device_cap[1]}) does not support bf16."
                        )
                        print(
                            "Falling back to fp32. Note: FP32 uses ~2x more VRAM than bf16."
                        )
                        actual_mixed_precision = "fp32"

                # Accelerator uses "no" for fp32
                accel_precision = (
                    "no" if actual_mixed_precision == "fp32" else actual_mixed_precision
                )
                ...
                if resume_checkpoint_path:
                    print(
                        f"Loading base model: {init_model_path} (will apply checkpoint weights from {resume_checkpoint_path})"
                    )
                else:
                ...
                if gradient_checkpointing:
                    ...
                    elif hasattr(qwen3tts.model, "talker") and hasattr(
                        qwen3tts.model.talker, "gradient_checkpointing_enable"
                    ):
                        qwen3tts.model.talker.gradient_checkpointing_enable()
                        print(
                            "Gradient checkpointing enabled on talker for VRAM optimization"
                        )
                else:
                ...
                if HAS_BNB and use_8bit_optimizer:
                    optimizer = bnb.optim.AdamW8bit(
                        qwen3tts.model.parameters(), lr=lr, weight_decay=weight_decay
                    )
                    ...
                    if not HAS_BNB:
                        print(
                            "Using standard AdamW (install bitsandbytes for lower VRAM usage)"
                        )
                    else:
                ...
                if resume_checkpoint_path:
                    ...
                    optimizer_state_path = os.path.join(
                        resume_checkpoint_path, "optimizer.pt"
                    )
                    if os.path.exists(optimizer_state_path):
                        ...
                        print(
                            "No optimizer state found, starting fresh (momentum will be reset)"
                        )

                    # Load scheduler state if using warmup
                    if scheduler:
                        scheduler_state_path = os.path.join(
                            resume_checkpoint_path, "scheduler.pt"
                        )
                        if os.path.exists(scheduler_state_path):
                            scheduler.load_state_dict(
                                torch.load(scheduler_state_path, map_location="cpu")
                            )
                            ...
                            if completed_steps > 0:
                                print(
                                    f"Fast-forwarding scheduler by {completed_steps} steps (no saved state found)"
                                )
                ...
                print(
                    f"Starting training from epoch {start_epoch + 1} to {end_epoch}..."
                )
                ...
                total_optimizer_steps = (
                    num_update_steps_per_epoch * end_epoch + resume_from_step
                )
                global_step = (
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                "output_subfolder": (
                    "STRING",
                    {"default": "Qwen3-TTS/output", "multiline": False},
                ),
            },
        }
...
class Qwen3LoadAudioFolder:
    ...
    def load_folder(self, folder_path):
        ...
        def process_wav(w):
            ...
            if w.ndim == 1:
                w = w[np.newaxis, :]  # (1, samples)
            if w.shape[0] > w.shape[1]:
                w = w.T  # Ensure (channels, samples)
...
class Qwen3AudioCompare:
            ...
            "required": {
                ...
                "speaker_encoder_model": (
                    base_models,
                    {
                        ...
                        "tooltip": "Base model to load speaker encoder from (only loads ~76 weights, not the full model)",
                    },
                ),
            },
            "optional": {
                "local_model_path": (
                    ...
                    {
                        "default": "",
                        "tooltip": "Optional custom path to model directory. If empty, uses default models/Qwen3-TTS/ location.",
                    },
                ),
            },
        }
    ...
    def _load_speaker_encoder(self, model_repo, local_model_path=""):
        ...
        if (
            Qwen3AudioCompare._speaker_encoder is not None
            and Qwen3AudioCompare._speaker_encoder_cache_key == model_path
        ):
            return Qwen3AudioCompare._speaker_encoder
        if not os.path.exists(model_path):
            raise ValueError(
                f"Base model not found at {model_path}. Please download it first using Qwen3-TTS Loader."
            )
        ...
        if "speaker_encoder_config" not in config_dict:
            raise ValueError(
                f"Model at {model_path} does not contain speaker_encoder_config. Only Base models (e.g., Qwen3-TTS-12Hz-0.6B-Base) include the speaker encoder."
            )
        ...
        print(
            f"Loaded speaker encoder from {model_repo} ({len(speaker_weights)} weights)"
        )
    ...
    def compare(
        ...
        speaker_encoder = self._load_speaker_encoder(
            speaker_encoder_model, local_model_path
        )
        ...
        target_sr = 24000
        if ref_sr != target_sr:
            ref_wav_mel = librosa.resample(
                ref_wav.astype(np.float32), orig_sr=ref_sr, target_sr=target_sr
            )
        else:
            ref_wav_mel = ref_wav
        if gen_sr != target_sr:
            gen_wav_mel = librosa.resample(
                gen_wav.astype(np.float32), orig_sr=gen_sr, target_sr=target_sr
            )
        else:
            gen_wav_mel = gen_wav
